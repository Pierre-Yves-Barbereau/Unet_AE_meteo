...
#        x = nn.functional.interpolate(x,size=(107,133),mode='bicubic')
#        x = self.lrelu(self.conv28_14(self.pad(x)))
#        print("x.shape = ",x.shape)
#        x = torch.cat((dm1,x),dim=1)
#        x = self.lrelu(self.conv28_14bis(self.pad(x)))
#        
#        x = nn.functional.interpolate(x,size=(214,267),mode='bicubic')
#        x = self.lrelu(self.conv14_7(self.pad(x)))
#        print("x.shape = ",x.shape)
#        x = torch.cat((dm2,x),dim=1)
        
        
        
        
#        x=self.lrelu(self.conv14_out(self.pad(x)))
        return x
    
    
#        d0=self.lrelu(self.conv56_56(self.pad(self.conv3_56(self.pad(nn.functional.interpolate(x,scale_factor=2,mode='bicubic'))))))
#        d1 = self.lrelu(self.conv56_56(self.pad(self.conv3_56(self.pad(x)))))
#        d2 = self.lrelu(self.conv56_112(self.pad(self.maxpool(d1))))
#        d3 = self.lrelu(self.conv112_224(self.pad(self.maxpool(d2))))
#        d4 = self.lrelu(self.conv224_448(self.pad(self.maxpool(d3))))
#        d5 = self.lrelu(self.conv448_896(self.pad(self.maxpool(d4))))
#        d6 = self.lrelu(self.conv896_1792(self.pad(self.maxpool(d5))))
#        
#        x = nn.functional.interpolate(d6,scale_factor=2,mode='bicubic')
#        x = self.lrelu(self.conv1792_896(self.pad(x)))
#        x = torch.cat((d5,x),dim=1)
...
#        
#        x = nn.functional.interpolate(x,scale_factor=2,mode='bicubic')
#        x = self.lrelu(self.conv224_112(self.pad(x)))
#        x = torch.cat((d2,x),dim=1)
#        x = self.conv224_112(self.pad(x))
#
#        x = nn.functional.interpolate(x,scale_factor=2,mode='bicubic')
#        x = self.lrelu(self.conv112_56(self.pad(x)))
#        x = torch.cat((d1,x),dim=1)
#        x = self.lrelu(self.conv112_56(self.pad(x)))
#        
#        x = nn.functional.interpolate(x,scale_factor=2,mode='bicubic')
#        x = torch.cat((d0,x),dim=1)
#        x = self.lrelu(self.conv56_3(self.pad(self.conv112_56(self.pad(x)))))
#        return x
#    
    
    
#        d0=self.lrelu(self.conv56_56(self.pad(self.conv3_56(self.pad(nn.functional.interpolate(x,scale_factor=2,mode='bicubic'))))))
#        d1 = self.lrelu(self.pad(self.conv56_56(self.conv3_56(self.pad(x)))))
#        d2 = self.lrelu(self.conv224_112(self.pad(shuffle_pooling(d1,2))))
#        d3 = self.lrelu(self.conv448_224(self.pad(shuffle_pooling(d2,2))))
#        d4 = self.lrelu(self.conv896_448(self.pad(shuffle_pooling(d3,2))))
#        d5 = self.lrelu(self.conv1792_896(self.pad(shuffle_pooling(d4,2))))
#        d6 = self.lrelu(self.conv3584_1792(self.pad(shuffle_pooling(d5,2))))
#        
#        x = nn.functional.interpolate(d6,scale_factor=2,mode='bicubic')
#        x = self.lrelu(self.conv1792_896(self.pad(x)))
#        x = torch.cat((d5,x),dim=1)
